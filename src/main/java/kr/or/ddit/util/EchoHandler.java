...
public class EchoHandler extends TextWebSocketHandler {
	...
	protected void handleTextMessage(WebSocketSession session, TextMessage message) 
		...
		// 수강신청 수강인원 실시간 업데이트 일 경우
		if (receiver.startsWith("lecCd=")) {
			sendLectureCnt(receiver);
		// 실시간 알림 일 경우
		} else {
			// 로그인 사용자의 실시간 알림
			if (receiver.equals("로그인사용자")) getNotificationAndSendClient(userCd);
			// 알림 수신자에게 알림 보내기
			else getNotificationAndSendClient(receiver);
		}
	}
	...
	private void sendLectureCnt(String receiver) 
			throws IOException {
	...
	private void getNotificationAndSendClient(String userCd)
			throws IOException {
		...
		for (WebSocketSession single : sessions) {
			...
			if (usedUserCd.equals(userCd)) {
				...
				TextMessage sendMsg = new TextMessage(etcUtil.getJsonList(ntfList));
